...
/**
 * Contains all operations that has to do with user files, it uses Microsoft Azure.
 */
@Service
public class DefaultUserFilesService implements UserFilesService {
	...
	public DefaultUserFilesService(BlobServiceClient blobServiceClient,
			BlobBatchClient blobBatchClient, FileShareDao fileShareDAO, long maxStoragePerUser,
			long maxFileSize) {
	...
	private String getCompressedJson(String content) throws BadRequestException {
	...
	@Override
	public boolean canUserCreateFileWithSize(String username, String filename, long fileSize) {
	...
	private String getBlobItemUrl(BlobItem blobItem, BlobContainerClient containerClient) {
	...
	@Override
	public void shareFileWith(User fileOwner, String filename, String sharedWithUsername,
		...
		BlobClient fileBlobClient = fileOwnerBlobContainerClient.getBlobClient(filename);

		if (!fileBlobClient.exists())
			throw new BadRequestException("couldn't find the file " + filename);

		boolean isFileAlreadyShard = fileShareDAO.isFileSharedWith(fileOwner.getUsername(),
				filename, sharedWithUsername);

		if (isFileAlreadyShard)
			throw new BadRequestException("the file is already shared!");

		fileShareDAO.shareFile(fileOwner.getUsername(), filename, sharedWithUsername, canEdit);
	}
	...
	@Override
	public void deleteShare(User fileOwner, String filename, String sharedWithUsername)
			throws BadRequestException {
	...
	@Override
	public void updateSharedFileAccess(User fileOwner, String filename, String sharedWithUsername,
